...
17. ✅ **Convergence criteria require a named mechanism, not a numbered taxonomy slot** — Earlier iterations required the LLM to map the converged mechanism onto a 12-category taxonomy (direct TF-TF contacts, DNA-mediated, chromatin/accessibility, cofactor sharing, 3D genome, phase separation, PTMs, GRNs, binding kinetics, RNA-mediated, molecular crowding, paralog cross-binding). This was too rigid — real mechanisms (e.g. DNA-methylation-driven competitor exclusion, found in the Apr 13 TEAD1/YY1 run) do not always fit a predefined bucket, and forcing them into one distorts the conclusion. The current rule in `src/prompts/summary.py` is: when converged, populate `mechanism_category_name` with a descriptive free-form name that fits the evidence, and set `mechanism_category_number` to `null`. The 12-category list survives as reference material only.
